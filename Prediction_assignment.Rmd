...
plot(test_Acc$Test_accuracy,x = test_Acc$PC_count)

print(test_Acc)

```
The best test error related to random forest created using all PC's.
The best test (out of sample error) is less then train error and equal to {r test_Acc[8,2]}
...
table(valid_class_36,valid_PC$classe)
valid_accuracy <- sum(valid_class_36 == valid_PC$classe)/length(valid_PC$classe)
```
Accuracy on validation data is: {r valid_accuracy}
